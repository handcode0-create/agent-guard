...
5. Aucune écriture automatique. Aucun commit automatique. La décision reste toujours humaine.
...
Active l'interrupteur **"Mode développeur"** en haut à droite de la page.

### 3. Charger l'extension

Clique sur le bouton **"Charger l'extension non empaquetée"** en haut à gauche et sélectionne le dossier `bridge/extension` situé à la racine de ce dépôt.

### 4. Configurer l'extension (Options)

1. Clique sur l'icône de l'extension **Agent Guard Bridge** dans ton navigateur.
2. Clique sur **"Ouvrir les options"**.
3. Remplis les champs requis :
* **URL du serveur local** : `http://127.0.0.1:4756` *(Il est recommandé d'utiliser l'IP brute `127.0.0.1` pour éviter les conflits IPv6 sous Windows)*.
* **Token partagé** : Rentre le jeton de sécurité de ton choix (il doit correspondre à celui du fichier `config.json`).


4. Clique sur **Enregistrer**.

### 5. Configurer le serveur (`config.json`)
...
*(⚠️ Sous Windows, pense à bien doubler les antislashs `\\` dans le chemin `projectRoot`)*.
...
5. Tu appliques uniquement ce que tu acceptez avec `agent-guard approve`.
